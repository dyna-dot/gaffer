 ...
 * graphId and  (if graphId is not known by the {@link
 * uk.gov.gchq.gaffer.store.library.GraphLibrary}) the {@link Schema} and {@link
 * StoreProperties}.
 *
...
public class FederatedStore extends Store {
  ...
  private Boolean isPublicAccessAllowed =
      Boolean.valueOf(IS_PUBLIC_ACCESS_ALLOWED_DEFAULT);
   ...
   */
  @Override
  public void initialise(final String graphId, final Schema unused,
                         final StoreProperties properties)
      throws StoreException {
    super.initialise(graphId, new Schema(), properties);
    customPropertiesAuths = getCustomPropertiesAuths();
    isPublicAccessAllowed =
        Boolean.valueOf(getProperties().getIsPublicAccessAllowed());
  }
   ...
   * @deprecated see {@link
   *     FederatedStoreUtil#updateOperationForGraph(Operation, Graph)}
   */
  @Deprecated
  public static <OP extends Operation>
      OP updateOperationForGraph(final OP operation, final Graph graph) {
   ...
   */
  public void addGraphs(final Set<String> graphAuths, final String addingUserId,
                        final boolean isPublic,
                        final GraphSerialisable... graphs)
      throws StorageException {
    addGraphs(graphAuths, addingUserId, isPublic,
              FederatedGraphStorage.DEFAULT_DISABLED_BY_DEFAULT, graphs);
  }
   ...
   * @param graphs            the graph to add
   * @param isPublic          if this class should have public access.
   * @param disabledByDefault true if the graph should be disabled by default -
   *     requiring the graphId to be provided in queries
   * @param graphAuths        the access auths for the graph being added
   * @throws StorageException if unable to put graph into storage
   */
  public void addGraphs(final Set<String> graphAuths, final String addingUserId,
    ...
    final FederatedAccess access = new FederatedAccess(
        graphAuths, addingUserId, isPublicAccessAllowed && isPublic,
        disabledByDefault);
    addGraphs(access, graphs);
  }

  public void addGraphs(final FederatedAccess access,
                        final GraphSerialisable... graphs)
      throws StorageException {
  ...
  public Schema getSchema(final Map<String, String> config,
  ...
  public Set<StoreTrait> getTraits(final GetTraits getTraits,
                                   final Context context) {
   ...
   */
  public Collection<Graph> getGraphs(final User user,
                                     final String graphIdsCsv) {
  ...
  @Override
  protected OutputOperationHandler<GetElements,
  ...
  @Override
  protected OutputOperationHandler<GetAllElements,
                                   CloseableIterable<? extends Element>>
  ...
  protected OutputOperationHandler<
      ? extends GetAdjacentIds, CloseableIterable<? extends EntityId>>
  ...
  private Set<String> getCustomPropertiesAuths() {
    ...
    return (Strings.isNullOrEmpty(value))
        ? null
        : Sets.newHashSet(getCleanStrings(value));
  }

  private void _add(final GraphSerialisable newGraph,
