...
/**
 * <p>
 * A {@code If} is an {@link Operation} which will execute one of two
 * Operations, based on the result of testing an input Object against a provided
 ...
 * which allows properties other than the input object to be passed to the
 * predicate, whilst preserving the initial input.
 * </p>
 * <p>
 ...
 * then the untouched original input is passed on to the operation determined by
 * the predicate test.
 * </p>
 * <p>
 * A simple boolean, or anything that resolves to a boolean, can also be used to
...
@JsonPropertyOrder(value = {"input", "condition", "conditional", "then",
                            "otherwise", "options"},
                   alphabetic = true)
public class If<I, O> extends GenericInput<I>
  ...
  public void updateOperations(final Collection<Operation> operations) {
    if (null == operations || 3 != operations.size()) {
      throw new IllegalArgumentException(
          "Unable to update operations - exactly 3 operations are required. Received " +
  ...
  public Operation getThen() { return then; }

  public void setThen(final Operation then) { this.then = then; }

  public Operation getOtherwise() { return otherwise; }
