...
public class JoinIT extends AbstractStoreIT {
  private List<Element> inputElements =
      new ArrayList<>(Arrays.asList(getJoinEntity(TestGroups.ENTITY_3, 1),
                                    getJoinEntity(TestGroups.ENTITY_3, 2),
                                    getJoinEntity(TestGroups.ENTITY_3, 3),
                                    getJoinEntity(TestGroups.ENTITY_3, 4),
                                    getJoinEntity(TestGroups.ENTITY_3, 6)));
  private List<Element> innerJoinElements =
      new ArrayList<>(Arrays.asList(getJoinEntity(TestGroups.ENTITY_3, 1),
                                    getJoinEntity(TestGroups.ENTITY_3, 2),
                                    getJoinEntity(TestGroups.ENTITY_3, 3),
                                    getJoinEntity(TestGroups.ENTITY_3, 4)));

  private final GetElements rhsGetElementsOperation =
      new GetElements.Builder()
          .input(new EntitySeed(VERTEX_PREFIXES[0] + 0))
          .view(new View.Builder().entity(TestGroups.ENTITY_3).build())
          .build();
  ...
  @Test
  public void shouldRightSideInnerJoinUsingKeyFunctionMatch()
      throws OperationException {

    // Given
    final Map map = new Map.Builder<>()
                        .input(Lists.newArrayList(4L))
                        .first(new Identity())
                        .build();

    final ArrayList<Entity> input =
        Lists.newArrayList(getJoinEntity(TestGroups.ENTITY, 4),
                           getJoinEntity(TestGroups.ENTITY_2, 4),
                           getJoinEntity(TestGroups.ENTITY_3, 4));

    final Join<Object> rightJoin =
        new Join.Builder<>()
            .flatten(true)
            .matchKey(MatchKey.RIGHT)
            .joinType(JoinType.INNER)
            .operation(map)
            .matchMethod(new KeyFunctionMatch(new Identity(),
                                              new ExtractProperty("count")))
            .input(input)
            .build();
    // When
    final Iterable<? extends MapTuple> rightResults =
        graph.execute(rightJoin, user);
    ...
    assertEquals(getJoinEntity(TestGroups.ENTITY, 4),
                 loadedResults.get(0).get(MatchKey.LEFT.name()));
    assertEquals(4L, loadedResults.get(0).get(MatchKey.RIGHT.name()));
    assertEquals(getJoinEntity(TestGroups.ENTITY_2, 4),
                 loadedResults.get(1).get(MatchKey.LEFT.name()));
    assertEquals(4L, loadedResults.get(1).get(MatchKey.RIGHT.name()));
    assertEquals(getJoinEntity(TestGroups.ENTITY_3, 4),
                 loadedResults.get(2).get(MatchKey.LEFT.name()));
    assertEquals(4L, loadedResults.get(2).get(MatchKey.RIGHT.name()));
  }

  @Test
  public void shouldLeftSideOuterJoinUsingKeyFunctionMatch()
      throws OperationException {

    // Given
    final Map map = new Map.Builder<>()
                        .input(Lists.newArrayList(4L, 1L, 2L))
                        .first(new Identity())
                        .build();
    ...
    final Join<Object> leftJoin =
        new Join.Builder<>()
            .flatten(true)
            .matchKey(MatchKey.LEFT)
            .joinType(JoinType.OUTER)
            .operation(map)
            .matchMethod(new KeyFunctionMatch(new ExtractProperty("count"),
                                              new ToLong()))
            .input(input)
            .build();
    // When
    ...
    assertEquals(3, loadedResults.size());
    assertEquals(getJoinEntity(TestGroups.ENTITY_4, 5),
                 loadedResults.get(0).get(MatchKey.LEFT.name()));
    assertEquals(null, loadedResults.get(0).get(MatchKey.RIGHT.name()));
    assertEquals(getJoinEntity(TestGroups.ENTITY_4, 6),
                 loadedResults.get(1).get(MatchKey.LEFT.name()));
    assertEquals(null, loadedResults.get(1).get(MatchKey.RIGHT.name()));
    assertEquals(getJoinEntity(TestGroups.ENTITY_5, 5),
                 loadedResults.get(2).get(MatchKey.LEFT.name()));
    assertEquals(null, loadedResults.get(2).get(MatchKey.RIGHT.name()));
  }

  @Test
  public void shouldRightSideFullJoinUsingKeyFunctionMatch()
      throws OperationException {

    // Given
    final Map map = new Map.Builder<>()
                        .input(Lists.newArrayList(2L, 1L, 2L, 3L))
                        .first(new Identity())
                        .build();

    final ArrayList<Entity> input =
        Lists.newArrayList(getJoinEntity(TestGroups.ENTITY, 1),
    ...
    final Join<Object> leftJoin =
        new Join.Builder<>()
            ...
            .operation(map)
            .matchMethod(new KeyFunctionMatch(new ToLong(),
                                              new ExtractProperty("count")))
            .input(input)
            .build();
    // When
    ...
    assertEquals(Lists.newArrayList(getJoinEntity(TestGroups.ENTITY_2, 2),
                                    getJoinEntity(TestGroups.ENTITY_2, 2)),
                 loadedResults.get(0).get(MatchKey.LEFT.name()));
    assertEquals(2L, loadedResults.get(0).get(MatchKey.RIGHT.name()));

    assertEquals(Lists.newArrayList(getJoinEntity(TestGroups.ENTITY, 1)),
                 loadedResults.get(1).get(MatchKey.LEFT.name()));
    ...
    assertEquals(Lists.newArrayList(),
                 loadedResults.get(3).get(MatchKey.LEFT.name()));
  ...
  public void shouldLeftKeyFullJoin() throws OperationException {
    ...
    // When
    final Iterable<? extends MapTuple> results =
        graph.execute(joinOp, getUser());
  ...
  public void shouldLeftKeyOuterJoin() throws OperationException {
    // Given
    final List<Element> expectedResults =
        new ArrayList<>(Arrays.asList(getJoinEntity(TestGroups.ENTITY_3, 6)));
  ...
  public void shouldRightKeyOuterJoin() throws OperationException {
    // Given
    final List<Element> expectedResults =
        new ArrayList<>(Arrays.asList(getJoinEntity(TestGroups.ENTITY_3, 8)));
  ...
  private void assertKeysExist(final Iterable<Element> expected,
                               final Iterable<? extends MapTuple> actual,
  ...
  private void addJoinEntityElements(final String group) {
    for (int i = 1; i <= 4; i++) {
      ...
      try {
        graph.execute(new AddElements.Builder().input(entity).build(),
                      getUser());
    ...
    try {
      graph.execute(
          new AddElements.Builder().input(getJoinEntity(group, 8)).build(),
          getUser());
    } catch (final OperationException e) {
      throw new RuntimeException(e.getMessage(), e);
    }
  ...
  private Entity getJoinEntity(final String group,
                               final Integer countProperty) {
    return new Entity.Builder()
        .group(group)
        .vertex(VERTEX_PREFIXES[0] + 0)
        .property(TestPropertyNames.SET, CollectionUtil.treeSet("3"))
        .property(TestPropertyNames.COUNT,
                  Long.parseLong(countProperty.toString()))
        .build();
  }
}
