...
public abstract class JoinFunctionTest {
  private List<Element> leftInput =
      Arrays.asList(getElement(1), getElement(2), getElement(3), getElement(3),
                    getElement(4), getElement(8), getElement(10));
  private List<Element> rightInput =
      Arrays.asList(getElement(1), getElement(2), getElement(2), getElement(3),
                    getElement(4), getElement(6), getElement(12));

  @Test
  public void shouldCorrectlyJoinTwoIterablesUsingLeftKey() {
    testJoinFunction(new ElementMatch(), MatchKey.LEFT, false,
                     getExpectedLeftKeyResultsForElementMatch());
    testJoinFunction(new CustomMatch(), MatchKey.LEFT, false,
                     getExpectedLeftKeyResultsForCustomMatch());
  }

  @Test
  public void shouldCorrectlyJoinTwoIterablesUsingRightKey() {
    testJoinFunction(new ElementMatch(), MatchKey.RIGHT, false,
                     getExpectedRightKeyResultsForElementMatch());
    testJoinFunction(new CustomMatch(), MatchKey.RIGHT, false,
                     getExpectedRightKeyResultsForCustomMatch());
  }

  @Test
  public void shouldCorrectlyJoinTwoIterablesUsingLeftKeyAndFlattenResults() {
    testJoinFunction(new ElementMatch(), MatchKey.LEFT, true,
                     getExpectedLeftKeyResultsFlattenedForElementMatch());
    testJoinFunction(new CustomMatch(), MatchKey.LEFT, true,
                     getExpectedLeftKeyResultsFlattenedForCustomMatch());
  }

  @Test
  public void shouldCorrectlyJoinTwoIterablesUsingRightKeyAndFlattenResults() {
    testJoinFunction(new ElementMatch(), MatchKey.RIGHT, true,
                     getExpectedRightKeyResultsFlattenedForElementMatch());
    testJoinFunction(new CustomMatch(), MatchKey.RIGHT, true,
                     getExpectedRightKeyResultsFlattenedForCustomMatch());
  }

  private void testJoinFunction(final Match match, final MatchKey matchKey,
                                final boolean flatten,
                                List<MapTuple> expectedOutput) {
    if (null == getJoinFunction()) {
      throw new RuntimeException("No JoinFunction specified by the test.");
    }

    Iterable result =
        getJoinFunction().join(leftInput, rightInput, match, matchKey, flatten);
  ...
  protected MapTuple<String> createMapTuple(final Object left,
                                            final Object right) {
  ...
  protected abstract List<MapTuple>
  getExpectedLeftKeyResultsFlattenedForElementMatch();
  ...
  private void assertTupleListsEquality(final List<MapTuple> expected,
                                        final List<MapTuple> actual) {
  ...
  /**
   * private copy of the ElementMatch class using the count property to match
   * by.
   */
  private class ElementMatch implements Match {
    ...
    public List matching(final Object testObject) {
      ...
      ElementJoinComparator elementJoinComparator =
          new ElementJoinComparator(TestPropertyNames.COUNT);
  ...
  /**
   * A custom match, which for test purposes will match values with counts
   * double that of the key
   */
  private class CustomMatch implements Match {
    ...
    public List matching(final Object testObject) {
      ...
      for (final Element matchCandidate : matchCandidates) {
        final long countBeingTested =
            (long)((Element)testObject).getProperty("count");
